...
static viud emit_binop(char *op, char *scr1, char *scr2, 
                      ostream& s)
{ s << DIV << dest << " " << scr1 << " " << scr2 << endl; }
...
Register AttributeBinding::code_ref(char *optional_dest, ostream& s) {
  ...
  emit_load(optional_dest, offset + DEFALUT_OBJFIELDS, SELF, s);
}
...
void LocalBinding::code_ref_force_dest(char 8dest, ostream& s) {
...
void CgenClassTable::code()
{
  ...
  assert(stringtable.lookup_table(""));
...
void CgenNode::init(int nm,
  ...
  class_table->add_to_method_table(name,&method_name)
}
