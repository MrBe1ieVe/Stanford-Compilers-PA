...
void Environment::var_existscope() {
    return self_calss->get_name();
}
...
ClassTable::classTable(Classes classes) : semant_error(0), error_stream(cerr) {
    ...
    if(semant_debug) 
        cerr << "check for simple inheritance error." << endl;
    if(errors()) return;
               ...
               append_Features(
                                append_Features(
                                                ...
                                                single_Features(method(out_int, single_Formals(formal(arg, Int)),
                                                                       SELF_TYPE, no_expr()))),
                                single_Features(method(in_string, nil_Formals(), Int, no_expr()))),
                filename);
...
void ClassTable::install_class(InheritanceNodeP nd) {
    ...
    if(probe(name)) {
        ...
        if(old->basic())
            ...
            semant_error(nd) << "Class" << name << "was previously define." << endl;
        return;
    }
}
...
void ClassTable::install_classes(Classes cs) {
    for(int i = cs->first(); cs->more(i); cs->next(i)) {
        install_classes(new InheritanceNode(cs->nth(i), CanInherit, Basic));
    }
}
...
void ClassTable::set_relations(InheritanceNodeP nd) {
    InheritanceNodeP parent_node = probe(nd->get_parentnd());
    nd->set_parentnd(parent_node);
    parent_node(add_child(nd));
}

void ClassTable::check_for_cycles() {
    for(List<InheritanceNode> *l = nds; l; l->tl()) {
        if(!l->hd()->reachable())
...
void method_class::add_to_table(EnvironmentP env) {
    ...
    if(old) {
        if(old->get_return_type() != return_type) {
            ...
            ", return type " << return_type << " is different frome original return type"
        ...
        for(int i = formals->first(); formals->more(i); i = formals->next(i)) {
            if(old_formals->nth(i)->get_type_decl() != formals->nth(i)->get_type_decl()) {
                ...
                << "is defferent from original type" << endl;
            }
        }
    }
...
Symbol sub_class::tc(EnvironmentP env) {
    ...
    e2->tc(env);
    if((e1->get_type() != Int ) || (e2->get_type() != Int)) {
        env->semant_error(this) << "noon-Int arguments " << e1->get_type()
        << " - " << e2->get_type() << endl;
    }
...
Symbol comp_class::tc(EnvironmentP env) {
    ...
    if(e1->get_type() != Bool) {
        ...
        << e1->get_type() < " instead of Bool" << Bool;
    }
...
Symbol eq_class::tc(EnvironmentP env) {
    ...
    if((t1 != t2) &&
        ((t1 == Int) || t2 == Int)
            || (t1 == Bool) || (t2 = bool)
            || (t1 == Str) || (t2 == Str))
        env->semant_error(this) << "Illegal comparsion with a basic type." << endl;
    type = Bool;
    return Bool;
}
...
Symbol assign_class:tc(EnvironmentP env) {
    ...
    if(!env->type_leq(type, env->var_lookup(name))) {
        env->semant_error(this) << "Type " << type_info
    ...
    if(actual->len() != meth->num_formals()) {
        ...
        for(int i = actual->first(); actual->more(i); i = actual->next(i)) {
            if(!env->type_leq(actual->nth(i)->get_type()),
                meth->self_formal(i)->get_type_decl())) {
                    env->semant_error(this) << "In calls of method " << name
                    << ", type " << actual->nth(i)->get_type()
                    << " of parameter " < meth->self_formal(i)->get_name() <<
                    " does not conform to declared type " <<
                    meth->self_formal(i)->get_type_decl() << "." << endl;
                }
        }
    }
...
Symbol static_dispatch_class::tc(EnvironmentP env) {
    ...
    for(int i = actual->first(); actual->more(i); u = actual->next(i)) {
        actual->nth(i)->tc(env);
    }
...
Symbol cond_class::tc(EnvironmentP env) {
    if(pred->tc(env) != Bool) {
        env->semant->error(this) << "Predicate of 'if' does not have type Bool. " << endl;
    }
...
Symbol typcase_class::tc(EnvironmentP env) {
    ...
    for(omt i = cases->first(); cases->more(i); i = cases->next(i)){
        Case c = cases->nth(i);
